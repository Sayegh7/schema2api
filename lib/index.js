module.exports = function(schema) {
  ...
  for (var table in schema) {
    if (schema.hasOwnProperty(table)) {
      ...
      var getOneQuery = `SELECT *  FROM ${table} WHERE ${primaryKey ||
        columns[0]}={{ data.params.${primaryKey || columns[0]} }};`;
      var deleteQuery = `DELETE  FROM ${table} WHERE ${primaryKey ||
        columns[0]}={{ data.params.${primaryKey || columns[0]} }};`;
      var editQuery = `UPDATE ${table} SET ${editValuesString} WHERE ${primaryKey ||
        columns[0]}={{ data.params.${primaryKey ||
        columns[0]} }};${getOneQuery}`;
      //   get all
      routes.push({
        method: "get",
        endpoint: "/" + table,
      ...
      routes.push({
        method: "get",
        endpoint: `/${table}/:${primaryKey || columns[0]}`,
        query: getOneQuery
      });
      //   delete
      routes.push({
